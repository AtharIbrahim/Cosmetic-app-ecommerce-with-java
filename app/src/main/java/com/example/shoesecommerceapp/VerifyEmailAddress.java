...
import com.example.shoesecommerceapp.databinding.ActivityVerifyEmailAddressBinding;
...
public class VerifyEmailAddress extends AppCompatActivity {
    ActivityVerifyEmailAddressBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityVerifyEmailAddressBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());

        dialog = new Dialog(VerifyEmailAddress.this);
        ...
        String VerifyEmail = getIntent().getStringExtra("VerifyEmail");
        if ("null".equals(VerifyEmail)) { // Use "null".equals(verifyEmail) for string comparison
            binding.EmailConfirmButton.setVisibility(View.VISIBLE);
        } else {
            binding.EmailConfirmButton.setVisibility(View.GONE);
        }
...
//            Toast.makeText(this, "Your account is locked. Please try again later.", Toast.LENGTH_SHORT).show();
        ...
        fetchUserData();
        binding.EmailConfirmButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                ...
                ProgressDialog progressDialog = new ProgressDialog(VerifyEmailAddress.this);
                progressDialog.setMessage("Your verification code: " + randomCode);
                progressDialog.setCancelable(true);
                progressDialog.show();

                // Pass the random code to the next activity
                Intent intent = new Intent(VerifyEmailAddress.this, EmailOTPVerification.class);
    ...
    private void fetchUserData() {
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                runOnUiThread(() -> {
                    dialog.dismiss();
                    Toast.makeText(VerifyEmailAddress.this, "Failed to fetch user data", Toast.LENGTH_SHORT).show();
                });
            }
                ...
                if (response.isSuccessful()) {
                    ...
                    try {
                        ...
                        if (user.has("error")) {
                            runOnUiThread(() -> {
                                dialog.dismiss();
                                Toast.makeText(VerifyEmailAddress.this, "Error", Toast.LENGTH_SHORT).show();
                            });
                        } else {
                            runOnUiThread(() -> {
                                try {
                                    dialog.dismiss();
                                    binding.editTextPhone2.setText(user.getString("Email"));
