...
import com.example.shoesecommerceapp.databinding.ActivityEmailOtpverificationBinding;
...
public class EmailOTPVerification extends AppCompatActivity {
    ActivityEmailOtpverificationBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dialog = new Dialog(EmailOTPVerification.this);
        ...
        binding.EmailConfirmOTPButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                dialog.show();
                String otp = binding.VerifyOtpText.getText().toString();
                if (TextUtils.isEmpty(otp)) {
                    dialog.dismiss();
                    binding.VerifyOtpText.setError("OTP is Empty");
                    return;
                }
                if (!otp.equals(String.valueOf(OTP))) {
                    dialog.dismiss();
                    binding.VerifyOtpText.setError("OTP Verification Failed..");
                    binding.VerifyOtpText.setText("");
                    handleFailedVerification(email);
                    return;
                }
                dialog.dismiss();
                Intent intent = new Intent(EmailOTPVerification.this, SuccessVerifyEmailAddress.class);
    ...
    private void updateUser(String email, String verifyEmail) {
        ...
        Request request = new Request.Builder()
                .url("http://192.168.169.98/ShoesAppECommerce/verifyemail.php")
                .post(formBody)
                .build();
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                runOnUiThread(() -> {
                    dialog.dismiss();
                    Toast.makeText(EmailOTPVerification.this, "Failed to update user data", Toast.LENGTH_SHORT).show();
                });
            }
    ...
    private void handleFailedVerification(String email) {
        ...
        if (failedAttempts >= MAX_FAILED_ATTEMPTS) {
            ...
            if (currentTime - lockTime < LOCK_DURATION_MILLIS) {
                ...
                dialog.dismiss();
                Intent intent = new Intent(EmailOTPVerification.this, VerifyEmailAddress.class);
                startActivity(intent);
                finish();
                return;
            } else {
